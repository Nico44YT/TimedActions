...
public class ResumeTimedActionCommand {
    public static ArgumentBuilder<ServerCommandSource, ?> create() {
        return CommandManager.literal("resume")
                .then(CommandManager.argument("target", EntityArgumentType.entity()).executes(ResumeTimedActionCommand::execute));
    ...
    public static int execute(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if(target != null) {
            if(target.getTimedAction().isPresent()) {
                ...
                target.resumeTimedAction();
                context.getSource().sendFeedback(() -> Text.literal(String.format("Resumed timed action (%s)", id)), true);
                return 0;
            }
        }
