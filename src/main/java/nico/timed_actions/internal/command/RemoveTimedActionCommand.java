...
public class RemoveTimedActionCommand {
    public static ArgumentBuilder<ServerCommandSource, ?> create() {
        return CommandManager.literal("remove")
                .then(CommandManager.argument("target", EntityArgumentType.entity()).executes(RemoveTimedActionCommand::execute));
    ...
    public static int execute(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        if(context.getArgument("target", EntitySelector.class).getEntity(context.getSource()) instanceof LivingEntity target) {
            if(target.getTimedAction().isPresent()) {
                ...
                target.removeTimedAction();
                context.getSource().sendFeedback(() -> Text.literal(String.format("Removed timed action (%s)", id)), true);
                return 0;
            }
        }
